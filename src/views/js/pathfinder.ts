...
var bathroomNodes: PathNode[] = [];

var cornerNodes: PathNode[] = [];
var roomNodes: PathNode[] = [];
...
class PathNode {
    ...
    constructor(xPos: number, yPos: number, id: string) {
        this.xPos = xPos;
        this.yPos = yPos;
        this.id = id;
        this.connections = [];
...
class Connection {
    ...
    constructor(nodeIdA: string, nodeIdB: string, walkTime: number = -1) {
...
class Path {
    ...
    constructor(nodes: PathNode[], walkTime: number) {
        this.nodes = nodes;
        this.walkTime = walkTime;
    }
}
...
async function setupPathfinder() {
    ...
    var accountData = $('#accountData').html()
...
function spliceTransform(transform: string) {
...
function pathfindToNearestBathroom(roomId: string, brPrefs: string[] = []) {
    var paths: Path[] = [];
...
function selectShortestPath(paths: Path[]) {
...
function findPath(startNodeId: string, endNodeId: string) {
    ...
    while (openSet.length > 0) {
        ...
        var currentNode: PathNode = openSet[0];
...
function retracePath(startNode: PathNode, endNode: PathNode) {
    var path = [];
...
function getDirectDistance(nodeA: PathNode, nodeB: PathNode) {
...
var pathToDraw: Path | null;
var currentPathIndex: number;
var currentLinePercent: number;
var currentLineLength: number;

var currentNodeA: PathNode;
var currentNodeB: PathNode;
...
function drawPathPeriodic() {
    ...
    if (currentLinePercent >= 1) {
        ...
        if (currentPathIndex >= pathToDraw.nodes.length) {
            var bathroom = pathToDraw.nodes[pathToDraw.nodes.length - 1];
...
function drawPathAnimated(path: Path) {
...
function drawPath(path: Path) {
    var bathroom = path.nodes[path.nodes.length - 1];
    drawCircle(bathroom.xPos, bathroom.yPos)
    ...
    for (var i = 0; i < path.nodes.length; i++) {        
        if (i == 0) 
            ...
            drawLineBetweenNodes(currentNodeA, currentNodeB);
    }
}

// draws a line connecting two nodes
function drawLineBetweenNodes(nodeA: PathNode, nodeB: PathNode) {
...
function modifyLine(lineID: string, x1: number, y1: number, x2: number, y2: number) {
...
function drawCircle(x: number, y: number) {
    ...
    circle.setAttributeNS(null, 'r', (lineWidth*1.2).toString());
